...
dim_dict = {'max_node_num': 0, 'max_GPU_num': 2, 'max_worker_num': 8, 'max_job_type': 8,
            'max_job_running': 20, 'max_job_gpu': 16}
# node_dict = {1: 'master', 2: 'node1', 3: 'node2', 4: 'node3'}
node_dict = {1: 'master'}
reward_interval = 1
job_resource_demand = {'vgg-16': {'GPUmem_demand': 5000, 'GPUutil_demand': 90, 'time_per_epoch': 80, 'time_weight': 24}, # MiB 90%
                       'vgg-32': {'GPUmem_demand': 5000, 'GPUutil_demand': 90, 'time_per_epoch': 65, 'time_weight': 40},
                       'resnet-8': {'GPUmem_demand': 5000, 'GPUutil_demand': 35, 'time_per_epoch': 85, 'time_weight': 40},
                       'resnet-16': {'GPUmem_demand': 5000, 'GPUutil_demand': 30, 'time_per_epoch': 45, 'time_weight': 120},
                       'unet-8': {'GPUmem_demand': 4500, 'GPUutil_demand': 45, 'time_per_epoch': 50, 'time_weight': 24},
                       'unet-16': {'GPUmem_demand': 4500, 'GPUutil_demand': 60, 'time_per_epoch': 30, 'time_weight': 60},
                        'cnn-16': {'GPUmem_demand': 1100, 'GPUutil_demand': 30, 'time_per_epoch': 140, 'time_weight': 33},
                       ...
                       'rnn-16': {'GPUmem_demand': 1300, 'GPUutil_demand': 45, 'time_per_epoch': 25, 'time_weight': 240},
                        'cnnl-16': {'GPUmem_demand': 1100, 'GPUutil_demand': 50, 'time_per_epoch': 30, 'time_weight': 80},
                       'cnnl-32': {'GPUmem_demand': 1100, 'GPUutil_demand': 60, 'time_per_epoch': 22, 'time_weight': 240},
                       'dcgan-16': {'GPUmem_demand': 1400, 'GPUutil_demand': 60, 'time_per_epoch': 22, 'time_weight': 40},
                        'dcgan-32': {'GPUmem_demand': 1400, 'GPUutil_demand': 60, 'time_per_epoch': 22, 'time_weight': 40},
                        'seq2seq-16': {'GPUmem_demand': 1100, 'GPUutil_demand': 60, 'time_per_epoch': 30, 'time_weight': 40},
                        'seq2seq-32': {'GPUmem_demand': 1100, 'GPUutil_demand': 60, 'time_per_epoch': 30, 'time_weight': 40},
                       }
...
def podBinding(k8sCoreV1api,podName, nodeName, namespace, gpu_index):
    ...
    if nodeName == 'master':
        meta.annotations = {'ALIYUN_COM_GPU_MEM_ASSIGNED': "false",
                            ...
                            'ALIYUN_COM_GPU_MEM_DEV': "40", 'ALIYUN_COM_GPU_MEM_IDX': str(gpu_index), # master要执行任务，所以增加MEM_DEV
...
def podScheduling(k8sCoreV1api, policy, job_tracker, job_epoch_records):
    ...
    for podName in policy:
        ...
        # 在这里对job_tracker中的两个列表进行添加，记录gpu_id和node_id的信息！！！！！！
...
def getNodeResource(k8sCoreV1api,api_client, nodeName):
    ...
    metrics = api_client.call_api(url, 'GET', auth_settings=['BearerToken'], response_type='json', _preload_content=False)
    # 将字节数据解码为UTF-8编码的字符串
...
# 作业调度及性能分析
def run(job_name_list, log_path, start_time, useNodeName, log, execution_time, job_tracker, share_info, agent, performance, main_writer, submission_dict):
    # 随机打乱列表中元素的顺序，该操作会改变原始列表而不返回新的列表
    ...
    submission.submit_list(node_index=0, name_list=submission_dict[0])  # 先提交0分组的作业 name_list是一个job_name的列表
    ...
    for j in range(10000000): # j代表时间步的变化
        ...
        if j>0 and submission_dict.__contains__(j):
            ...
            '''
            submisson_dict的键是服从指数分布的随机数
            OrderedDict([(0, ['resnet-2-6-20-8', 'dcgan-7-6-10-32']), (6, ['resnet-2-6-30-16']),
             (10, ['vgg-1-3-30-16']), (15, ['resnet-2-4-20-16']), (19, ['rnn-5-4-10-16']),
             (27, ['unet-3-4-20-8']), (32, ['unet-3-4-10-16']), (39, ['vgg-1-2-20-16']),
             (41, ['unet-3-3-30-8'])])
            '''
        ...
        # from multiprocessing import Manager （多进程管理器对象）
        # multiprocessing.Manager创建和管理共享对象，以便多个进程间进行通信和共享数据
        man = Manager() #
        ...
        for node in node_info:
            ...
            main_writer.write_usage(node, np.mean(np.array(node_info[node]['GPUmemory_util']))/np.mean(np.array(node_info[node]['GPUmemory_total'])),
        ...
        for i in k8sCoreV1api.list_namespaced_pod(namespace).items: # i是V1Pod
            ...
            job_id = i.metadata.owner_references[0].name # name就是TFJob的name
            ...
            temp_pod_info['creation_time'] = (i.metadata.creation_timestamp + datetime.timedelta(hours=8)).strftime('%Y-%m-%d %H:%M:%S')
            resource_demand_reference = job_meta[0]+ '-' + job_meta[4]  # model_name-batch_size
            ...
            if i.status.phase == "Succeeded": # Pod中的所有容器都已经成功终止，不会重新启动
                ...
                if i.status.container_statuses[0].state.terminated.finished_at is not None:
                    job_info[job_id]['end_time'].append((i.status.container_statuses[0].state.terminated.finished_at + datetime.timedelta(
                ...
                if job_info[job_id]['end_time'][0] and job_id not in execution_time:
                    execution_time[job_id] = (datetime.datetime.strptime(job_info[job_id]['end_time'][0], '%Y-%m-%d %H:%M:%S')
                ...
                if i.status.phase == "Failed":
                    ...
                    if job_info[job_id]['end_time'][0] and job_id not in execution_time:
                        execution_time[job_id] = (datetime.datetime.strptime(job_info[job_id]['end_time'][0], '%Y-%m-%d %H:%M:%S') -
                                                  datetime.datetime.strptime(job_info[job_id]['start_time'][0], '%Y-%m-%d %H:%M:%S')).seconds
                        for k in range(len(job_tracker[job_id]['gpu_id'])):
                            hostinfo[node_ip_dict[node_dict[job_tracker[job_id]['node_id'][k]]]]['GPUmemory_util'][
        ...
        if j > 0:
            ...
            for job_id in job_epoch_records:
                if job_epoch_records[job_id] is not None and job_id in job_info and job_info[job_id]['epoch'][0] is not None and \
                        any(phase == 'Running' for phase in job_info[job_id]['phase']):
                    # 如果以上条件都满足，说明该作业正在运行且具有相关的状态信息
                    # job_epoch_records
                    job_epoch_records[job_id][str(j)] = performance.state_extraction(node_info, job_info, job_id, job_resource_demand)
        performance.update_model(num_epochs=10) # 训练的总轮次：10 更新模型 保存模型参数和训练样本到对应路径

        # 以下代码统计每个节点上正在运行（所有的阶段都是Running）的作业数量
        ...
        for job_id in job_tracker:
            ...
            time_list = [int(x) for x in temp_job] # temp_job此时是什么？？？？？
            if len(time_list) == 0:
                continue
            ...
            if start_epoch_num and end_epoch_num and last_time > first_time and end_epoch_num > start_epoch_num:
                ...
                if str(j - reward_interval) in job_epoch_records[job_id]:
                    ...
                    # TODO
                    real_value = (last_time-first_time)/(end_epoch_num-start_epoch_num)  #  ???为什么这样计算real_value 一个epoch的时间？？
                    # 将实际执行时间添加到性能分析中
                    ...
                    tuned_loss_list.append((max(abs(predict_value - real_value)-2, 0.0001)/real_value)) # 经过调整的损失值，调整绝对误差
        ...
        if real_list and loss_list and tuned_loss_list:
            main_writer.write_pm(np.mean(np.array(real_list)), np.mean(np.array(loss_list)), np.mean(np.array(tuned_loss_list)))
        ...
        print(log_time)
        td = (datetime.datetime.strptime(log_time, '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(curr_time, '%Y-%m-%d %H:%M:%S')).seconds
        if td < 180:
          time.sleep(180 - td)
...
def main(share_info):
    ...
    for i in range(10):
        ...
        run(job_name_list, log_path, start_time, useNodeName, log, execution_time, job_tracker, share_info, agent, performance, main_writer, submission_dict)
        time.sleep(120)
...
if __name__ == '__main__': # 确保代码只在主程序中执行，而不是在被导入为模块时执行
    ...
    p2 = multiprocessing.Process(target=main, args=(share_info, ),name="job_scheduler") # 执行主任务
    p1.start()
    time.sleep(10) # 让主进程休眠300s，等待p1进程执行一段时间后再启动p2
    p2.start()
    p2.join() # 确保在主进程退出之前，进程p2已经完成任务
    gc.collect()
