...
COPY autoselect/pyproject.toml autoselect/poetry.lock autoselect/
RUN cd autoselect && poetry install --no-root --no-interaction --no-cache
COPY autoselect autoselect
RUN cd autoselect && poetry install --no-interaction --no-cache
...
RUN useradd autoselect -m -u 1000 -d /home/autoselect

COPY --from=builder --chown=autoselect:autoselect ${VIRTUAL_ENV} ${VIRTUAL_ENV}
COPY --from=builder autoselect autoselect
