...
class AutoArgumentParser(ArgumentParser):
    ...
    def parse_args_into_dataclasses(
        ...
        if return_remaining_strings:
            ...
            if remaining_args:
                raise ValueError(f"Some specified arguments are not used by the AutoArgumentParser: {remaining_args}")
    ...
    def parse_dict(self, args: Dict[str, Any], allow_extra_keys: bool = False) -> Tuple[DataClass, ...]:
        ...
        if not allow_extra_keys and unused_keys:
            raise ValueError(f"Some keys are not used by the AutoArgumentParser: {sorted(unused_keys)}")
