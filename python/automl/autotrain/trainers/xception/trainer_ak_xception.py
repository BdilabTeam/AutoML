...
from .configuration_xception import XceptionTrainerConfig
...
class AKXceptionMainTrainer:
    def __init__(
        self,
        config: XceptionTrainerConfig,
        **kwargs
    ):
        ...
        if config.mp_pretrained:
            xception_params["pretrained"] = config.mp_pretrained
        output_node = ak.XceptionBlock(**xception_params)(output_node)
...
class AKXceptionForImageClassificationTrainer(BaseTrainer):
    def __init__(self, config: XceptionTrainerConfig, **kwargs) -> None:
        if config.task_type != TaskType.IMAGE_CLASSIFICATION.value:
            raise ValueError(f"Task type '{config.task_type}' mismatch, expected task type is '{TaskType.IMAGE_CLASSIFICATION.value}'")
        super().__init__(config=config)

        self.trainer = AKXceptionMainTrainer(config=config)
...
class AKXceptionForImageRegressionTrainer(BaseTrainer):
    def __init__(self, config: XceptionTrainerConfig, **kwargs):
